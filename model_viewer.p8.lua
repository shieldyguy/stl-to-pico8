...
-- --- communic8 Lua Library End ---

-- message handling from web interface -- REMOVED old handler
...
-- communic8 RPC update function
local update_communic8
...
function _init()
  ...
  -- --- communic8 Initialization Start ---
  local functions = {}

  -- RPC 0: receive_model
  functions[0] = {
    input={
      arg_types.array(arg_types.tuple({arg_types.number, arg_types.number, arg_types.number})), -- vertices
      arg_types.array(arg_types.tuple({arg_types.number, arg_types.number, arg_types.number}))  -- faces
    },
    output={},
    execute=function(args)
      printh("received model via communic8")
      local vertices_data = args[1]
      local faces_data = args[2]

      local new_vertices = {}
      for v_tuple in all(vertices_data) do
        add(new_vertices, {x=v_tuple[1], y=v_tuple[2], z=v_tuple[3]})
      end

      local new_faces = {}
      for f_tuple in all(faces_data) do
        -- Ensure faces are integer indices
        add(new_faces, {flr(f_tuple[1]), flr(f_tuple[2]), flr(f_tuple[3])})
      end
  ...
  functions[1] = {
    ...
    output={},
    execute=function(args)
      printh("set_wireframe RPC received: " .. tostr(args[1])) -- Debug print
      wireframe = args[1]
      return {}
    end
  }
  ...
  functions[2] = {
    ...
    output={},
    execute=function(args)
      printh("set_fill RPC received: " .. tostr(args[1])) -- Debug print
      draw_filled = args[1]
      return {}
    end
  }
  ...
  functions[3] = {
    ...
    output={},
    execute=function(args)
      printh("set_color RPC received: " .. tostr(args[1])) -- Debug print
      model_color = args[1]
      return {}
    end
  }
...
function _update()
  ...
  -- Call communic8 update loop
  if (update_communic8) update_communic8()
end
